...
export const Container = styled.div<containerStyleProps>`
  ...
  background-color: ${({ bgColor }) =>
    bgColor ? colors[bgColor] : colors.transparent};

  max-width: ${({ maxW }) =>
    maxW ? (typeof maxW === "number" ? `${maxW}px` : sizes[maxW]) : "1280px"};
  max-height: ${({ maxH }) =>
    maxH ? (typeof maxH === "number" ? `${maxH}px` : sizes[maxH]) : ""};

  min-height: ${({ minH }) =>
    minH ? (typeof minH === "number" ? `${minH}px` : sizes[minH]) : ""};

  height: ${({ h }) =>
    h ? (typeof h === "number" ? `${h}px` : sizes[h]) : ""};
  width: ${({ w }) =>
    w ? (typeof w === "number" ? `${w}px` : sizes[w]) : "100%"};
  ...
  border-color: ${({ border }) =>
    border === "none" ? "none" : border && colors[border.color]};
